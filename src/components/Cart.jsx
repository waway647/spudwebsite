...
const Cart = ({ isOpen, toggleCart, cartItems, subtotal, onQuantityChange, onRemoveItem }) => {
  return (
    <div
      className={`fixed right-0 top-0 h-full 2xl:w-[25%] xl:w-[30%] lg:w-[35%] md:w-[50%] bg-[#FFFBF8] shadow-xl transform transition-transform duration-300 z-50 ${
          ...
          {cartItems.map((item) => (
            <div key={item.id} className="flex justify-between gap-10 items-start border-b border-gray-200 pb-6">
              ...
              <div className="flex flex-col text-right">
                <p className="font-dela-gothic-one text-lg">₱{item.price}</p>
                <button 
